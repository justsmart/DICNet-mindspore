...
def train_DIC(mul_X, mul_X_val, WE,WE_val,yv_label,args):
    ...
    # optimizer = nn.Adam(params=model.trainable_params(), learning_rate=args.lrkl, weight_decay=0.0)
    ...
    for epoch in range(int(args.maxiter)):
        ...
        for batch_idx in range(int(np.ceil(num_X / args.batch_size))):
            ...
            def forward_fn(mul_X_batch,we,sub_obrT,sub_target,fan_sub_target):
                ...
                fusion_loss = loss_CL*1 + args.gamma * loss_AE  + loss_Cont * args.beta
        ...
        # print('train ovel')
        # scheduler.step()
        yp_prob = test_DIC(model, mul_X_val, WE_val, args)
        ...
        if epoch > 300 and ( (best_value_result[4]-value_result[4]>0.03) or
                best_value_result[4] < args.min_AP or (abs(total_loss_last - total_loss) < 1e-7 or delta_y < args.tol)):
...
def test_DIC(model, mul_X_test, WE_test, args):
    ...
    for batch_idx in range(int(np.ceil(num_X_test / args.batch_size))):
        ...
        we = WE_test[idx]
        # _, linshi_q, _, _ = model(mul_X_test_batch[0],mul_X_test_batch[1],mul_X_test_batch[2],mul_X_test_batch[3],mul_X_test_batch[4],mul_X_test_batch[5], we)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--maxiter', default=20, type=int) ## 50 for pascal07 20 for mirflickr 300 for others
    parser.add_argument('--batch_size', default=128, type=int)
    parser.add_argument('--dataset', type=str, default='mirflickr_six_view')
    parser.add_argument('--dataPath', type=str, default='data/mirflickr')
    ...
    parser.add_argument('--min_AP', default=0.13, type=float)
    ...
    pre_beta = [1e-3] #1e-3 is recommended for mirflickr and 1e-4 for pascal07
    pre_gamma = [1e-1]  
    best_AUC_me = 0
    best_AUC_mac = 0
    best_AP = 0
                ...
                for alpha in pre_alpha:
                    ...
                    for beta in pre_beta:
                        ...
                        for fnum in range(Pre_fnum):
                            ...
                            model, _,ap_loss = train_DIC(mul_X, mul_X_val, WE, WE_val, yv_label, args)
                            print('train over')
                            yp_prob = test_DIC(model, mul_X_rtest, WE_rtest, args)
                            print('test over')
